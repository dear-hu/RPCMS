...
class Temp extends Base{
	...
	public function setting(){
		...
		$tempDefault=Db::name('config')->where('cname = "template"')->field('cvalue')->find();
		if($tempDefault['cvalue'] != $temp){
			rpMsg('模板未启用，请先启用后再设置');
		}
		$sendpost=intval(input('sendpost')) == 1 ? 1 : 0;
		$cfg=Db::name('config')->where('cname = "temp_'.$tempDefault['cvalue'].'"')->field('cvalue')->find();
		if($sendpost == 1){
			$data=input('post.');
			unset($data['sendpost']);
			if($cfg){
				$res=Db::name('config')->where('cname = "temp_'.$tempDefault['cvalue'].'"')->update(array('cvalue'=>addslashes(json_encode($data))));
			}else{
				$res=Db::name('config')->insert(array('cname' => 'temp_'.$tempDefault['cvalue'],'cvalue'=>addslashes(json_encode($data))));
			}
			Cache::update('template');
