...
class LogsMod{
	...
	public function select(){
		...
		$list=Db::name('logs')->alias('a')->join(array(
			array('category b','a.cateId=b.id','left'),
			array('user c','a.authorId=c.id','left'),
		))->where($this->whereArr)->where($this->whereStr)->field('a.id,a.title,a.authorId,a.cateId,a.excerpt,a.content,a.tages,a.isTop,a.views,a.comnum,a.upnum,a.upateTime,a.createTime,a.extend,a.status,b.cate_name as cateName,c.nickname as author')->limit(($this->page-1)*$this->limit.','.$this->limit)->order($this->order)->select();
		foreach($list as $k=>$v){
	...
	public function neighbor($logId){
		...
		$prev=Db::name('logs')->alias('a')->where($where1)->where($this->whereStr)->field('a.id,a.title,a.authorId,a.cateId,a.excerpt,a.content,a.tages,a.views,a.comnum,a.upnum,a.upateTime,a.createTime,a.extend')->order($order1)->find();
		$next=Db::name('logs')->alias('a')->where($where2)->where($this->whereStr)->field('a.id,a.title,a.authorId,a.cateId,a.excerpt,a.content,a.tages,a.views,a.comnum,a.upnum,a.upateTime,a.createTime,a.extend')->order($order2)->find();
		if(!empty($prev)){
	...
	*/
	public function related($logData=array(),$type='cate',$limit=10){
		...
		$list=Db::name('logs')->alias('a')->join(array(
			array('category b','a.cateId=b.id','left'),
			array('user c','a.authorId=c.id','left'),
		))->where($this->whereArr)->where($this->whereStr)->field('a.id,a.title,a.authorId,a.cateId,a.excerpt,a.content,a.tages,a.isTop,a.views,a.comnum,a.upnum,a.upateTime,a.createTime,a.extend,a.status,b.cate_name as cateName,c.nickname as author')->limit('0,'.$limit)->order($this->order)->select();
		foreach($list as $k=>$v){
			$list[$k]['extend'] =json_decode($v['extend'],true);
			$list[$k]['url'] = Url::logs($v['id']);
			$list[$k]['cateUrl'] = Url::cate($v['cateId']);
